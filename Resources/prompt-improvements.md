# Just some notes Pete is taking

## PROMPT.md

If doing any UI or design work, please load the brand design skill and the frontend design skill.

Use context7 if you need to research documentation.

## Spec-building prompts:

I'm building a [what you're building]. please analyze [notes or spec file] and help me  
flesh out a new spec.md file. For the spec.md file, let's try NOT to have a ton of code snippets, OR a full implementation    
plan. Rather this document is a COMPLETE list of user (me) preferences, decisions made, features & sub   
feature breakdown, the purpose of the app, etc. analyze those docs and create a new spec.md file         

please analyze the @context/spec.md file. Is it missing anything? Do you have any clarifying           
questions? The next step is building an implementation plan, so this document needs to ACCURATELY        
reflect my preferences and decisions